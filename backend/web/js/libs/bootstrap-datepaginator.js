...
;(function($, window, document, undefined) {
	...
	DatePaginator.prototype = {
		...
		setSelectedDate: function(date, format) {
			this._setSelectedDate(moment(date, format ? format : this.options.selectedDateFormat), false);
			this._render();
		},
		...
		_setSelectedDate: function(selectedDate, activateFunction) {
			...
			if ((!selectedDate.isSame(this.options.selectedDate)) &&
				...
				this.$element.trigger('selectedDateChanged', [selectedDate.clone(), activateFunction]);
			}
		},

		_back: function() {
			this._setSelectedDate(this.options.selectedDate.clone().subtract('day', 1), true);
			this._render();
		},

		_forward: function() {
			this._setSelectedDate(this.options.selectedDate.clone().add('day', 1), true);
			this._render();
	    },

	    _select: function(date) {
			this._setSelectedDate(moment(date, this.options.selectedDateFormat), true);
			this._render();
	    },

	    _calendarSelect: function(event) {
			this._setSelectedDate(moment(event.date), true);
			this._render();
		},
